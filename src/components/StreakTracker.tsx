      ...
      <div className="flex-1 flex flex-col justify-center">
        <div className="grid grid-flow-col grid-rows-7 gap-[6px] justify-between">
          {streakData.map((day, i) => (
            ...
            >
              {/* Tooltip on hover */}
              <div className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-aura-dark text-aura-cream text-[10px] md:text-xs px-2 py-1 rounded whitespace-nowrap pointer-events-none z-50">
                {day.intensity} entries on {day.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
              </div>
            </motion.div>
          ))}
        </div>
